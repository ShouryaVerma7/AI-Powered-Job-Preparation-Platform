...
const Interview = () => {
    ...
    useEffect(() => {
        if (interviewId) {
            getReportById(interviewId).catch(() => {
                // Report not found or unauthorized, go home
                navigate('/home')
            })
        }
    }, [interviewId])
    ...
    const scoreLabel =
        report.matchScore >= 80 ? 'Strong match for this role' :
            report.matchScore >= 60 ? 'Moderate match — some gaps' : 'Needs improvement'

    const scoreLabelClass =
        report.matchScore >= 80 ? 'label--high' :
            report.matchScore >= 60 ? 'label--mid' : 'label--low'
            ...
            <div className='interview-layout'>
                ...
                <nav className='interview-nav'>
                    ...
                    <button
                        ...
                        className={`button primary-button pdf-btn ${pdfLoading ? 'pdf-btn--loading' : ''}`}
                        disabled={pdfLoading}
                    >
                        {pdfLoading ? (
                            <>
                ...
                <aside className='interview-sidebar'>
                    ...
                    <div className='skill-gaps'>
                        ...
                        {report.skillGaps.length === 0 ? (
                            <p className='skill-gaps__empty'>No significant skill gaps detected 🎉</p>
                        ) : (
